...
to use the DreamDevBox and Docker in general.
...
Do not start the DreamDevBox with ``sudo`` or as ``root`` user. If it complains about permissions when
...
Whenever you want to stop the DreamDevBox, change configuration and start up again, do not forget to
